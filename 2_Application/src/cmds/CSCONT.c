...
#define KEY_CSCONT "CSCONT"

LOCAL void cmd_CSCONT(char* recv_buf, uint32_t dest_fd, SEND_BUF send_buf_fun)
{
	char *case_name;
	...
	send_buf_fun(dest_fd, "%s ", KEY_CSCONT);
	...
	if(case_item){
		if(state != CASE_STATE_PAUSE){
			send_buf_fun(dest_fd, "FAIL\r\n%s %s %s\r\n", KEY_CSCONT, case_name, casestate);
			goto end;
		}

		ret = send_countinu_misson(case_item);
		if(ret != RET_OK){
			send_buf_fun(dest_fd, "FAIL\r\ncode:\r\n", ret);
			goto end;
		}
		send_buf_fun(dest_fd, "%s %s continue\r\n", KEY_CSCONT, case_name);
	}else{
...
#define CMDOBJ_CSCONT {"CSCONT", "continue case.", cmd_CSCONT}
