...
/*    char file_path[FILE_PATH_LEN];*/
    ...
    for(i=0; i<cali_obj_num; i++){
		...
		flag = config_get_array_bool(cali_obj, "ADJ_PHA_BY_ATT", i, 0);
		if(flag){
			filename = config_get_array_string(cali_obj, "ADJ_PHA_BY_ATT_FILE", i, "");
			csv_cal_read_file(filename, (File_reader)get_inf_cal_data, cal_info);
		}

		flag = config_get_array_bool(cali_obj, "ADJ_PHA_BY_TABLE", i, 0);
		if(flag){
			filename = config_get_array_string(cali_obj, "ADJ_PHA_BY_TABLE_FILE", i, "");
			csv_cal_read_file(filename, (File_reader)get_pha_cal_data, cal_info);
		}

		flag = config_get_array_bool(cali_obj, "ADJ_PHA_BY_STRETCH", i, 0);
		if(flag){
			stretch_point = config_get_array_int(cali_obj, "ADJ_PHA_BY_STRETCH_POINT", i, 0);
			filename = config_get_array_string(cali_obj, "ADJ_PHA_BY_STRETCH_FILE", i, "");
